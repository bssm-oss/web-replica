...
func NewRootCmdWithConfig(cfg RootConfig) *cobra.Command {
	...
	cmd.PersistentFlags().StringVar(&opts.Fidelity, "fidelity", "high", "Visual fidelity mode: standard or high")
	cmd.PersistentFlags().BoolVar(&opts.AllowOwnedAssets, "allow-owned-assets", true, "Allow same-origin and CDN-subdomain image/font downloads")
